...
RSpec.describe TanksController, :type => :controller do
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      before(:each) do
        put :update, :id => @tank.id,:tank => {:title => 'Nuevo Alchup', :description => 'Nueva descripcion', :latitude => 1, :longitude => 2, :notes => 'Nueva nota'}
      end
      it "changes tank attributes" do
        @tank.reload
        expect(@tank.title).to eq("Nuevo Alchup")
        expect(@tank.description).to eq("Nueva descripcion")
        expect(@tank.latitude).to eq(1)
        expect(@tank.longitude).to eq(2)
